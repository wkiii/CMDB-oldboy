...
class NIC(BasePlugin):
    ...
    def win(self, handler, hostname):
        ret = handler.cmd('ipconfig')[0:20]
        return ret

    def linux(self, handler, hostname):
        repsone = BaseResponse()
        try:
            if self.debug:
                # 读取文件获取执行命令的结果
                with open(os.path.join(self.path, 'nic.out'), encoding='utf-8') as f:
                    ret = f.read()
                    interfaces_info = self._interfaces_ip(ret)
            else:
                interfaces_info = self.linux_interfaces(handler, hostname)
            self.standard(interfaces_info)
            repsone.data = interfaces_info
    ...
    def linux_interfaces(self, handler, hostname=None):
        '''
